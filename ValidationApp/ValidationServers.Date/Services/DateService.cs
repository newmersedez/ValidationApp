...
using ValidationServers.Date.Context;
...
public class ValidationDateService : ValidationDate.ValidationDateBase
{
    ...
    public ValidationDateService()
    {
        container.Register<IValidator, DateValidator>();
    ...
    {
        _rabbit_model.QueueDeclare(queue: "date", durable: false, exclusive: false, autoDelete: false, arguments: null);
        ...
        _rabbit_model.BasicPublish(exchange: "", routingKey: "date", basicProperties: null, body: body);
        _rabbit_model.BasicConsume(queue: "date", autoAck: true, consumer: _consumer);
